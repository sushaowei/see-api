...
class FileCache extends Object
{
    ...
    {
        parent::init();
        $this->cachePath = \See::getAlias($this->cachePath);
        if (!is_dir($this->cachePath)) {
            if (!mkdir($this->cachePath, $this->dirMode, true)) {
                trigger_error("create cache dir  {$this->path} failed, no permmission");
            }
        }
    }
    ...
    public function set($key, $value, $duration=0)
    {
        ...
        if (@file_put_contents($cacheFile, $value, LOCK_EX) !== false) {
            ...
            \See::$app->warning("Unable to write cache file '{$cacheFile}': {$error['message']}". __METHOD__);
            return false;
        }
    }
            ...
            while (($file = readdir($handle)) !== false) {
                ...
                if (is_dir($fullPath)) {
                    ...
                    if (!$expiredOnly) {
                        if (!@rmdir($fullPath)) {
                            $error = error_get_last();
                            \See::$log->warning("Unable to remove directory '{$fullPath}': {$error['message']}" .  __METHOD__);
                        }
                    }
                } elseif (!$expiredOnly || $expiredOnly && @filemtime($fullPath) < time()) {
                    if (!@unlink($fullPath)) {
                        $error = error_get_last();
                        \See::$log->warning("Unable to remove file '{$fullPath}': {$error['message']}" .  __METHOD__);
                    }
                }
            }
            closedir($handle);
